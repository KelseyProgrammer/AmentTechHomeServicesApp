...
export async function POST(req: NextRequest) {
  ...
  const { firstName, lastName, email, phone, address, propertyType, preferredDate, notes, hearAbout, services, total, carePlan, hasCustomQuote, aiProjectBrief } = payload
  ...
  const serviceLines = services
    .map(s => `<tr><td style="padding:6px 12px;border-bottom:1px solid #ede8e0">${escapeHtml(s.name)}</td><td style="padding:6px 12px;border-bottom:1px solid #ede8e0;color:#b89b6e;font-weight:bold">${s.price === 0 ? 'Custom' : `From $${s.price.toLocaleString()}`}</td></tr>`)
    .join('')
  ...
  const totalRow = hasCustomQuote
    ? `<tr><td style="padding:10px 12px;font-weight:700;color:#d4b98a">Pricing</td><td style="padding:10px 12px;font-weight:700;color:#d4b98a;font-size:16px">Custom Quote — Pricing after consultation</td></tr>`
    : `<tr><td style="padding:10px 12px;font-weight:700;color:#d4b98a">Estimated Starting Total</td><td style="padding:10px 12px;font-weight:700;color:#d4b98a;font-size:18px">From $${total.toLocaleString()}</td></tr>`

  const aiBriefSection = hasCustomQuote && aiProjectBrief
    ? `<div style="background:#f5ede0;border-radius:10px;padding:20px 24px;margin-bottom:24px;">
  <div style="font-family:Georgia,serif;font-size:16px;font-weight:600;color:#1e2d47;margin-bottom:12px;">🤖 AI Project Brief</div>
  <pre style="white-space:pre-wrap;font-size:13px;color:#1a1a2e;margin:0;font-family:inherit;">${escapeHtml(aiProjectBrief)}</pre>
</div>`
    : ''
...
<body style="font-family:'Lato',Arial,sans-serif;background:#f7f3ed;margin:0;padding:0;">
  <div style="max-width:600px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(30,45,71,0.12);">
    ...
    <div style="background:#1e2d47;padding:28px 32px;text-align:center;">
      ...
      <div style="margin-top:16px;font-size:18px;color:#d4b98a;font-weight:600;">${hasCustomQuote ? '🤖 New Custom Quote Request' : '📋 New Booking Request'}</div>
    </div>
    ...
    <div style="padding:28px 32px;">
      <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
        ...
        <tr>
          ...
          <td style="padding:8px 12px;color:#1e2d47"><a href="mailto:${escapeHtml(email)}" style="color:#1e2d47">${escapeHtml(email)}</a></td>
        </tr>
        <tr style="background:#f7f3ed;">
          ...
          <td style="padding:8px 12px;color:#1a1a2e"><a href="tel:${escapeHtml(phone)}" style="color:#1e2d47">${escapeHtml(phone)}</a></td>
        </tr>
        <tr>
          ...
          <td style="padding:8px 12px;color:#1a1a2e">${escapeHtml(address)}</td>
        </tr>
        ${propertyType ? `<tr style="background:#f7f3ed;"><td style="padding:8px 12px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#7a7a8a">Property Type</td><td style="padding:8px 12px;color:#1a1a2e">${escapeHtml(propertyType)}</td></tr>` : ''}
        ...
        ${hearAbout ? `<tr><td style="padding:8px 12px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#7a7a8a">Heard About Us</td><td style="padding:8px 12px;color:#1a1a2e">${escapeHtml(hearAbout)}</td></tr>` : ''}
      ...
      <p style="font-size:13px;color:#7a7a8a;line-height:1.6;">
        Reply directly to this email or call <a href="tel:${escapeHtml(phone)}" style="color:#b89b6e">${escapeHtml(phone)}</a> to confirm the appointment.
      </p>
    </div>
...
`
  ...
  const customerPriceLine = hasCustomQuote
    ? `<p style="margin:12px 0 0;font-size:14px;font-style:italic;color:#7a7a8a;">Custom-scoped services — Sarah will provide a quote after reviewing your brief.</p>`
  ...
  <div style="max-width:560px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(30,45,71,0.12);">
    ...
    <div style="padding:32px;">
      ...
      <p style="font-size:15px;color:#4a4a5a;line-height:1.7;text-align:center;margin-bottom:28px;">
        Hi ${escapeHtml(firstName)}, thanks for booking with Ament Home &amp; Tech Services!<br>
        ${hasCustomQuote
          ? 'Sarah will review your project brief and reach out within <strong>24 hours</strong> with a custom quote.'
      ...
      <div style="background:#f7f3ed;border-radius:10px;padding:20px 24px;">
        ...
        ${services.map(s => `<p style="margin:4px 0;font-size:14px;color:#1a1a2e">• ${escapeHtml(s.name)}</p>`).join('')}
        ${customerPriceLine}
      </div>
...
`
  ...
  try {
    ...
    const recipientEmail = (process.env.BOOKING_RECIPIENT_EMAIL || 'chrisament45@gmail.com').split(',').map((e: string) => e.trim())
    const fromEmail = process.env.BOOKING_FROM_EMAIL || 'onboarding@resend.dev'

    const subjectPrefix = hasCustomQuote ? 'Custom Quote Request' : 'New Booking Request'
    ...
    const r1 = await resend.emails.send({
      ...
      to: recipientEmail,
      reply_to: email,
      subject: `${subjectPrefix} — ${firstName} ${lastName} (${services.length} service${services.length !== 1 ? 's' : ''})`,
      html,
    })
    ...
    const r2 = await resend.emails.send({
      from: fromEmail,
      to: email,
      subject: hasCustomQuote
        ? "Your Ament Custom Quote Request — We'll be in touch!"
        : "Your Ament Service Request — We'll be in touch!",
      html: customerHtml,
    })
